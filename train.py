...
import mindsporevision.transforms as transforms
import mindsporevision.datasets as datasets
...
# Use CUDA
use_cuda = mindspore.cuda.is_available()
...
class MyDataset(mindspore.utils.data.Dataset):
...
class MyvailDataset(mindspore.utils.data.Dataset):
    def __init__(self, transform):
...
def cross_entropy(outputs, smooth_labels):
    loss = mindspore.nn.KLDivLoss(reduction='batchmean')
...
def main():
    ...
    for arch in [ 'wideresnet','preactresnet18']:
        ...
        if args['scheduler_name'] != None:
            scheduler = mindspore.optim.lr_scheduler.__dict__[args['scheduler_name']](optimizer,
